...
static void log_entire_pbuf(struct pbuf *p) {
	struct pbuf *q = p;

	LOG_VERBOSE("Received TCP request (tot_len=%u):\r", (unsigned int)p->tot_len);
	while (q != NULL) {
		if (q->payload != NULL && q->len > 0U) {
			LOG_VERBOSE("%.*s", (int)q->len, (const char *)q->payload);
		}
		q = q->next;
	}
	LOG_VERBOSE("EOF\r");
 ...
 * Should contain a JSON object with "mode" and "data" fields.
...
tcp_server_error_t parse_request(struct tcp_pcb *newpcb, struct pbuf *p) {
	...
	cJSON *action = cJSON_GetObjectItemCaseSensitive(json, "mode");
	if (!cJSON_IsString(action) || (action->valuestring == NULL)) {
		cJSON_Delete(json);
		return TCP_SERVER_ERR_VAL;
	}
	cJSON *data = cJSON_GetObjectItemCaseSensitive(json, "data");
	if (!cJSON_IsObject(data) && (!cJSON_IsString(data) || (data->valuestring == NULL))) {
		cJSON_Delete(json);
		return TCP_SERVER_ERR_VAL;
	}
...
tcp_server_error_t parse_init(struct tcp_pcb *newpcb, cJSON *data) {
	...
	if (cJSON_IsArray(data)) {
		for (uint8_t i = 0; i < cJSON_GetArraySize(data); i++) {
			...
			if (!cJSON_IsNumber(cJSON_GetObjectItemCaseSensitive(item, "row")) ||
					!cJSON_IsNumber(cJSON_GetObjectItemCaseSensitive(item, "data"))) {
		...
		cJSON_AddItemToObject(resp, "data", cJSON_Duplicate(data, 1));
...
tcp_server_error_t parse_led(struct tcp_pcb *newpcb, cJSON *data) {
	...
	LOG_DEBUG("LED mode changed to %s\r", cJSON_GetStringValue(data));
